...
- 取得元キーは装置ごとに異なります。
- ZEISS Crossbeam 550 が出力する TIFF ファイルには、メタデータ構造が異なる2種類（Auto / Manual）が存在します。
- ここでの Auto / Manual は TIFFメタデータ形式の違いを示します。

##### Crossbeam（CB550 / CB1540）

| 項目名 | 取得元                                                      | 日本語名  | 英語名 | データ型 | 単位 | 初期値 | 備考 |
| -- | ----- | ----------------- | -- | ------ | -- | --- | ---------------- |
| machine | image.machine（CB550 Auto）<br>detection.sem（CB550 Manual）<br>systemval.serial_number（CB1540）  | 装置名 | Machine | string |  | |  |
| date  | application.date（CB550 Auto）<br>aperture.date（CB550 Manual / CB1540） | データ取得日  | Date  | string |  | | format:date-time |
| raw_file_name | systemval.file_name（CB550 / CB1540）                      | 元ファイル名  | Raw File Name | string |  | |  |
| detector  | detection.detector_channel（CB550 Manual / CB1540）        | 検出器 | Detector  | string |  | |  |
| imaging_mode  | image.detector（CB550 Auto）<br>detection.signalaz0 / signalaz2 / signalaz3（CB550 Manual）  | 像モード  | Imaging Mode  | string |  | |  |
| acc_voltage | image.acc_voltage（CB550 Auto）<br>aperture.actualkv（CB550 Manual / CB1540）  | 加速電圧  | Acc. Voltage  | number | V  | |  |
| aperture_size | image.aperture_size_value（CB550 Auto）<br>aperture.aperture_size（CB550 Manual / CB1540） | 絞りサイズ | Aperture Size | number | m  | |  |
| esb_grid  | aperture.esb_grid / aperture.esb_grid_is（CB550 Manual / CB1540）  | EsBグリッドの値 | EsB Grid  | number |  | |  |
| wd  | aperture.wd（CB550 Manual / CB1540）                       | ワークディスタンス | WD  | number |  | |  |
| scan_speed  | detection.scanrate（CB550 Manual / CB1540）                | スキャンスピード  | Scan speed  | number |  | |  |
| dwel_time | scan.dwell（CB550 Auto）<br>detection.dwell_time（CB550 Manual / CB1540）  | デュエルタイム | Dwel Time | number | s  | |  |
| noise_reduction | detection.noise_reduction（CB550 Manual / CB1540）         | Noise reduction | Noise reduction | string |  | |  |
| noise_reduction_value | scan.lineavg（CB550 Auto）<br>aperture.nr_coeff（CB550 Manual / CB1540） | Noise reductionの値 | Noise reduction value | number |  | |  |
| frame_time  | tilepositionhistory.tilepositionentry.time（CB550 Auto）<br>aperture.frame_time（CB550 Manual / CB1540） | 像取得時間 | Frame Time  | number |  | |  |
| image_size  | detection.image_store（CB550 Manual）                      | 像サイズ  | Image Size  | number |  | |  |
| image_size_x  | tilepositionhistory.tilepositionentry.x（CB550 Auto）      | 像サイズ(X方向) | Image Size (X)  | number |  | |  |
| image_size_y  | tilepositionhistory.tilepositionentry.y（CB550 Auto）      | 像サイズ(Y方向) | Image Size (Y)  | number |  | |  |
| pixel_resolution  | scan.ux（CB550 Auto）<br>aperture.pixel_size（CB1540 / CB550 Manual）  | ピクセル分解能 | Pixel Resolution  | number | nm | |  |
| image_width | scan.fov_x（CB550 Auto）<br>aperture.width（CB550 Manual / CB1540）  | 像サイズ（幅） | Image Width | number | um | |  |
| image_height  | scan.fov_y（CB550 Auto）<br>aperture.height（CB550 Manual / CB1540） | 像サイズ（高さ）  | Image Height  | number | um | |  |
| magnification | aperture.mag（CB1540 / CB550 Manual）                      | 倍率  | Magnification | number | X  | |  |
...
| 項目名 | 取得元 | 日本語名  | 英語名 | データ型 | 単位 | 初期値 | 備考 |
| -- | ---- | ----------------- | -- | ------ | -- | --- | ---------------- |
| machine | system.system_type  | 装置名 | Machine | string |  | |  |
| date  | user.date | データ取得日  | Date  | string |  | | format:date-time |
| raw_file_name | raw_file_name | 元ファイル名  | Raw File Name | string |  | |  |
| detector  | detectors.name  | 検出器 | Detector  | string |  | |  |
| imaging_mode  | detectors.mode  | 像モード  | Imaging Mode  | string |  | |  |
| acc_voltage | beam.hv | 加速電圧  | Acc. Voltage  | number | V  | |  |
| aperture_size | ebeam.aperture_diameter | 絞りサイズ | Aperture Size | number | m  | |  |
| wd  | ebeam.wd  | ワークディスタンス | WD  | number |  | |  |
| dwel_time | escan.dwell | デュエルタイム | Dwel Time | number | s  | |  |
| noise_reduction_value | escan.line_integration  | Noise reductionの値 | Noise reduction value | number |  | |  |
| frame_time  | escan.frame_time  | 像取得時間 | Frame Time  | number |  | |  |
| image_size_x  | image.resolution_x  | 像サイズ(X方向) | Image Size (X)  | number |  | |  |
| image_size_y  | image.resolution_y  | 像サイズ(Y方向) | Image Size (Y)  | number |  | |  |
| pixel_resolution  | escan.pixel_width | ピクセル分解能 | Pixel Resolution  | number | nm | |  |
| image_width | escan.hor_fieldsize | 像サイズ（幅） | Image Width | number | um | |  |
| image_height  | escan.ver_fieldsize | 像サイズ（高さ）  | Image Height  | number | um | |  |
...
| sem | manufacturer | 装置メーカー名 | string | 'jeol_maiml' or 'jeol_fe' or 'tiff_exif' | |
...
| TIFF_EXIF | `path` | `dict_key` | ネスト辞書のドットパス(`path`)で値を取り出し、`dict_key`名で出力します。 |


## dataset関数の説明（SEM）

設定ファイルを読み込み、SEMデータの製造元（manufacturer）に応じて処理モードを切り替えます。
対応しているモードは **JEOL_fe モード、JEOL_maiml モード、TIFF_EXIF モード**の3種類です。
...
ZEISS製またはThermo Fisher製のSEM装置から出力される **tif形式データ**を用いた構造化処理を行います。
...
tif画像から抽出したEXIF情報（サイズ、ビット深度等）を
...
```
また、tif_info.json に保存された情報から mapping.csv を使って、取得したい項目だけをテンプレートで使用するメタデータ名に変換して出力できます。  
これにより、主要メタデータ定義に合わせつつ、**EXIF由来の特殊項目は主要項目から除外されつつ利用可能**です。
