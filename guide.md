...
- JEOLのMaiML/TXT形式、ZEISSのTIFF形式に対応したメタ情報の抽出、SEM画像の登録を行う。
...
- DT0023:
    ...
    - 【データセットテンプレートの説明】ZEISSのSEMをご利用の方に適したモードです。TIFF形式でデータを取得している方にご利用いただけます。SEMの専門家が監修したメタ情報をtifファイルから自動的に抽出し格納します。熱間圧延によるSEM（走査型電子顕微鏡）画像を、構造化処理に適した形式で扱うためのデータセットテンプレートです。
    - 【バージョン】1.0
    - 【データセット種別】加工・計測レシピ型
    - 【データ構造化】あり (システム上「あり」を選択)
    - 【取り扱い事業】NIMS研究および共同研究プロジェクト (PROGRAM)
