...
- JEOLの.maiml/.TXTフォーマット、ZEISSの.tifフォーマットに対応したメタ情報の抽出、SEM画像の登録を行う。
- JEOL_maimlテンプレートとZEISSテンプレートのみMultiDataTileモード対応
...
- DT0021:
    ...
    - 【データセットテンプレートの説明】JEOLのSEMをご利用の方に適したモードです。maimlフォーマットでデータを取得されている方がご利用いただけます。 SEMの専門家によって監修されたメタ情報をmaimlファイルから自動的に抽出し格納します。maimlファイル中に記載されたresultTemplate_semResultImage に記載された画像ファイルを代表画像として登録します。
    - 【バージョン】1.0
    - 【データセット種別】加工・計測レシピ型
...
- DT0023:
    - 【データセットテンプレートID】NIMS_DT0023_SEM_ZEISS_v1.0
    - 【データセットテンプレート名日本語】SEM ZEISS データセットテンプレート
    - 【データセットテンプレート名英語】SEM ZEISS dataset-template
    - 【データセットテンプレートの説明】ZEISSのSEMをご利用の方に適したモードです。tifフォーマットでデータを取得している方にご利用いただけます。SEMの専門家が監修したメタ情報をtifファイルから自動的に抽出し格納します。熱間圧延によるSEM（走査型電子顕微鏡）画像を、構造化処理に適した形式で扱うためのデータセットテンプレートです。
    - 【バージョン】1.0
    - 【データセット種別】加工・計測レシピ型
    - 【データ構造化】あり (システム上「あり」を選択)
    - 【取り扱い事業】NIMS研究および共同研究プロジェクト (PROGRAM)
